...
def main():
 ...
 grid=[SwingParams('oversold_uptrend',a,b,h,stop,False) for a in (0,1,2,3)
  for b in (5,10,15,20,25) for h in (2,3,5,8) for stop in (6,8,10)]
 folds=[];oos=[]
 for end in range(480,len(x)-59,60):
  train=x.iloc[max(0,end-480):end];test=x.iloc[end:end+60];rank=[]
  for p in grid:
   r=evaluate(train,p);score=r['expectancy_r']*min(1,r['count']/20)+r['max_dd_pct']/100 if r['count']>=10 else -999
 ...
 result={'folds':folds,'oos':agg,'gate':{'passed':bool(agg['count']>=25 and agg['expectancy_r']>0 and agg['pf']>=1.2)}}
 (ROOT/'uptrend_oversold.json').write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(result,ensure_ascii=False))
